/**
 * anusha-job-digest.ts — Daily summary for Anusha's job search pipeline.
 */
...
export function generateAnushaJobDigest(date?: string): string {
  ...
  let digest = `## Anusha — Job Search Report — ${targetDate}\n\n`;
...
const isMain = process.argv[1]?.includes("anusha-job-digest");
if (isMain) {
  ...
  if (cmd === "init") {
    ...
    console.log(JSON.stringify(store.getJobStats(), null, 2));
  } else {
    // Default: print digest
    const date = cmd || undefined;
    console.log(generateAnushaJobDigest(date));
  }
}
