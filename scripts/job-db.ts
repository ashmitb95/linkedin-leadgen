 ...
 *
 * Separate DB file (db/jobs.db) from the leads pipeline.
 */
...
export function updateJobNotes(id: string, notes: string): boolean {
  const db = getDb();
  const now = new Date().toISOString();
...
export function logJobRunStart(): number {
  const db = getDb();
  const now = new Date().toISOString();
...
export function getJobStats(): any {
  const db = getDb();
